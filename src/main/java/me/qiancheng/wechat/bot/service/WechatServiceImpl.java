...
import me.qiancheng.wechat.bot.robot.MoLiRobot;
import me.qiancheng.wechat.bot.robot.Robot;
import me.qiancheng.wechat.bot.util.Matchers;
...
public class WechatServiceImpl implements WechatService {
			...
			if (msgType == 51) {
				...
				if (Constant.FILTER_USERS.contains(msg.getString("ToUserName"))) {
					...
					if (msg.getString("FromUserName").startsWith("@@")) {
						...
						name = getUserRemarkName(peopleContent[0]);
						content = peopleContent[1];
						LOGGER.info("群聊消息 | " + name + " : " + peopleContent[1].replace("<br/>", "\n"));
					}
				...
				webwxsendmsg(wechatMeta, "橙酱还不支持图片呢", msg.getString("FromUserName"));
			} else if (msgType == 34) {
				webwxsendmsg(wechatMeta, "橙酱还不支持语音呢", msg.getString("FromUserName"));
			} else if (msgType == 42) {
	...
	private String getUserRemarkName(String id) {
		String name = null;
		// Contact
		for (int i = 0, len = Constant.CONTACT.getContactList().size(); i < len; i++) {
			JSONObject member = Constant.CONTACT.getContactList().get(i).asJSONObject();
